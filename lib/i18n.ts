/**
 * Bilingual EN/AR dictionary + helpers.
 *
 * Default locale is English (LTR). Users can flip to Arabic (RTL) via the
 * header toggle — choice persists in localStorage and is applied to
 * <html dir/lang> on mount + change.
 */

export type Locale = "en" | "ar";

export const LOCALES: readonly Locale[] = ["en", "ar"] as const;
export const DEFAULT_LOCALE: Locale = "en";
export const STORAGE_KEY = "pa_locale";

export const dirOf = (locale: Locale): "ltr" | "rtl" =>
  locale === "ar" ? "rtl" : "ltr";

export const otherLocale = (locale: Locale): Locale =>
  locale === "ar" ? "en" : "ar";

/* ────────────────────────────────────────────────────────────────
   Dictionary
   ──────────────────────────────────────────────────────────────── */

type Leaf = string;
type Tree = { [key: string]: Leaf | Tree };

export const dict: Record<Locale, Tree> = {
  en: {
    common: {
      brand: "Property-Agent",
      tagline: "AI-powered WhatsApp lead engine for brokers",
      back: "Back",
      cancel: "Cancel",
      save: "Save",
      submit: "Submit",
      continue: "Continue",
      loading: "Loading…",
      retry: "Try again",
      copy: "Copy",
      copied: "Copied",
      learnMore: "Learn more",
      egypt: "Cairo",
    },
    nav: {
      home: "Home",
      pricing: "Pricing",
      login: "Sign in",
      signup: "Sign up",
      signOut: "Sign out",
      getStarted: "Get started",
      dashboard: "Dashboard",
    },
    languageToggle: {
      label: "Language",
      switchTo: "العربية",
      currentEn: "EN",
      currentAr: "AR",
    },
    hero: {
      eyebrow: "AI broker · WhatsApp-native",
      titlePart1: "Your bot replies to clients,",
      titleAccent: "delivers ready leads",
      titlePart2: "the second they ask.",
      body:
        "Plug in your catalog in 30 seconds. The bot speaks Egyptian Arabic like a seasoned Cairo broker. You only get pinged when a customer is ready to talk.",
      ctaPrimary: "Start free — 30 sec",
      ctaSecondary: "Have an account? Sign in",
      pill1Title: "More leads, less time",
      pill1Body: "Real conversations with real customers, around the clock.",
      pill2Title: "Smart, natural replies",
      pill2Body: "Answers like a senior broker would — never robotic.",
      pill3Title: "Right on WhatsApp",
      pill3Body: "Where your customers already are. No new app to install.",
      badge1: "30-sec setup",
      badge2: "EG Arabic replies",
      badge3: "Email · WhatsApp alerts",
      mockup: {
        contact: "Property-Agent",
        contactStatus: "online",
        today: "TODAY",
        greeting: "Hi 👋 How can I help today? May I have your name?",
        greetingTime: "11:40 AM",
        msg1Customer: "I'm Ahmed — looking for a 3-bedroom in New Cairo, around 4M",
        msg1Time: "11:42 AM",
        msg2Bot: "Pleased to meet you, Ahmed. I have 4 great fits within your budget — should I send the first one?",
        msg2Time: "11:42 AM",
        msg3Customer: "Yes please",
        msg3Time: "11:43 AM",
        msg4ListingTitle: "Fifth Settlement",
        msg4ListingArea: "165 sqm",
        msg4ListingRooms: "3 BR · 2 BA",
        msg4ListingPrice: "EGP 3,800,000",
        msg4ListingTag: "Garden view",
        msg5Bot: "Includes a 12 sqm garden, 2 reserved parking spots, and a pool right next to the building.",
        msg5Time: "11:43 AM",
        msg6Customer: "I really like this one 💚",
        msg6Time: "11:44 AM",
        msg7Bot: "Glad to hear! When would suit you for a viewing?",
        msg7Time: "11:44 AM",
        msg8Customer: "Tomorrow morning works",
        msg8Time: "11:44 AM",
        msg9Bot: "Tap below to confirm — your broker will be ready 🗓️",
        msg9Time: "11:45 AM",
        ctaButton: "Reserve appointment",
        msg10Customer: "Yes, please reserve it for me",
        msg10Time: "11:46 AM",
        msg11Bot: "All set ✅ Your viewing is reserved. Our team will reach out shortly to confirm the booking.",
        msg11Time: "11:46 AM",
        composerHint: "Type a message",
        successTitle: "Viewing booked!",
        successWhen: "Tomorrow · 9:30 AM",
        successPhone: "+20 10 XXX XXXX",
        successFooter: "Lead added to your dashboard",
      },
    },
    how: {
      eyebrow: "How it works",
      title: "Three steps between you and your first lead.",
      step1Title: "Sign up & upload inventory",
      step1Body:
        "Fill the signup form, upload your properties (CSV), and pick a notification channel — email or WhatsApp.",
      step1Note: "We send you a broker key and a ready-to-share WhatsApp link.",
      step2Title: "Share your WhatsApp link",
      step2Body:
        "It includes your broker code. Drop it in your ads, Stories, business cards — every client who taps it lands with you directly.",
      step2Note: "No WhatsApp setup needed on your side.",
      step3Title: "Receive ready clients",
      step3Body:
        "The bot understands the client, recommends from your catalog, and when they're ready it pings you with their number and last message.",
      step3Note:
        "Dashboard with all leads, status filters, and quick actions.",
      quote:
        "“The difference between a good broker and a great one is reply speed. The bot replies in seconds and only hands you the ready ones.”",
      ctaStart: "Start signup →",
    },
    footer: {
      city: "Property-Agent · Cairo",
      version: "v1",
    },
    auth: {
      loginEyebrow: "Sign in",
      loginTitlePart: "Back to your",
      loginTitleAccent: "broker desk",
      loginBody:
        "Use the email + password you set during signup. We'll drop you back on your dashboard in seconds.",
      noAccountQuestion: "Don't have an account yet?",
      noAccountCta: "Sign up as a broker",
      credsTitle: "Email & password",
      credsEyebrow: "Your details",
      emailLabel: "Email",
      passwordLabel: "Password",
      loginSubmit: "Sign in",
      signinSuccess: "Welcome back",
      apiKeyEyebrow: "Alternative",
      apiKeyTitle: "Sign in with API key",
      apiKeyBody:
        "For legacy accounts that don't have a password yet. Use the same key shown after signup.",
      apiKeySubmit: "Sign in with key",
      apiKeySwitch: "Use an API key instead →",
      apiKeyInvalid: "Key is invalid or no longer valid",
      genericError: "Something went wrong, try again",
      wrongCreds: "Invalid email or password",

      signupEyebrow: "Create your broker account",
      signupTitlePart: "Build your",
      signupTitleAccent: "client engine",
      signupTitleEnd: "in under a minute",
      signupBody:
        "Email, password, business name, phone, channel. We send your broker key and a ready WhatsApp link the moment you submit.",
      businessName: "Business name",
      brokerEmail: "Email",
      brokerPhone: "Phone (with country code)",
      passwordPlaceholder: "8 characters or more",
      confirmPassword: "Confirm password",
      channel: "Notification channel",
      channelEmail: "Email",
      channelWhatsApp: "WhatsApp",
      channelBoth: "Both",
      submitSignup: "Create my account",
      passwordsMustMatch: "Passwords don't match",
      passwordTooShort: "At least 8 characters",
      emailTaken: "This email already has an account",
      signupSuccess: "Welcome aboard",

      successTitle: "You're in — your broker dashboard is ready",
      successBody:
        "Your magic link is below. Share it everywhere and clients land straight in your bot.",
      whatsappCta: "Open in WhatsApp",
      goToDashboard: "Go to dashboard",
      step1: "Account",
      step2: "Business",
      step3: "Channel",
      stepOf: "Step {current} of {total}",
      next: "Continue",
      previous: "← Back",
      stepAccountTitle: "Create your login",
      stepAccountBody: "Email + password — you'll use these every time you sign in.",
      stepBusinessTitle: "Tell us about your office",
      stepBusinessBody: "We need this to label customers when they reach you.",
      stepChannelTitle: "How should we reach you?",
      stepChannelBody: "We'll ping you the moment a customer is ready to talk.",
    },
    dashboard: {
      eyebrow: "Broker desk",
      title: "Your office on",
      titleAccent: "WhatsApp",
      tabStats: "Overview",
      tabLeads: "Leads",
      tabInventory: "Inventory",
      tabKnowledge: "Knowledge",
      tabAnalytics: "Analytics",
      tabSettings: "Settings",
      loadFailed: "Couldn't load data",
      shareLink: "Your WhatsApp link",
      copyLinkShort: "Copy link",
      pageTitle: {
        overview: "Overview",
        leads: "Leads",
        inventory: "Inventory",
        knowledge: "Market knowledge",
        analytics: "Analytics",
        settings: "Settings",
      },
      pageSubtitle: {
        overview: "Snapshot of your activity, hot leads, and conversion funnel.",
        leads: "Every customer the bot handed to you.",
        inventory: "Your properties — what the bot recommends to customers.",
        knowledge: "Curated answers the bot grounds on for expert questions.",
        analytics: "Deeper trends — channels, response time, status mix.",
        settings: "Office details and notification preferences.",
      },
      sidebar: {
        collapse: "Collapse",
        expand: "Expand",
      },
      stats: {
        today: "Today",
        last7: "Last 7 days",
        last30: "Last 30 days",
        notifySuccess: "Notify success",
        todayEmpty: "No leads today yet",
        noNotificationsYet: "No notifications yet",
        recentTitle: "Recent leads",
        recentEnLabel: "Recent activity",
        emptyTitle: "No leads yet",
        emptyHint: "When a customer asks to be contacted, they'll appear here.",
        unknownCustomer: "Customer",
        hotLeads: "Hot leads",
        hotLeadsHint: "Need action now",
        totalLeads: "Total leads",
        totalLeadsHint: "All time",
        conversionRate: "Conversion",
        conversionRateHint: "Closed / total",
        avgResponse: "Avg response",
        avgResponseHint: "New → Notified",
        closedMtd: "Closed (MTD)",
        closedMtdHint: "This month",
        vsYesterday: "vs yesterday",
        vsLastWeek: "vs last week",
        vsLastMonth: "vs last month",
        seconds: "sec",
        minutes: "min",
        hours: "hr",
        days: "d",
        noData: "—",
      },
      funnel: {
        eyebrow: "Lead progression",
        title: "Conversion funnel",
        empty: "No leads yet — funnel will populate as customers arrive.",
        stage: {
          received: "Received",
          notified: "Notified",
          contacted: "Contacted",
          closed: "Closed",
        },
        dropFromPrev: "−{n}",
      },
      analytics: {
        loading: "Crunching the numbers…",
        empty: "No data yet — the charts will fill out as customers reach you.",
        leadsPerDay: {
          title: "Leads per day",
          subtitle: "Last 30 days · count of new leads per day.",
        },
        statusMix: {
          title: "Status distribution",
          subtitle: "How your funnel is currently spread.",
        },
        channelMix: {
          title: "Notification channels",
          subtitle: "Where the bot pinged you when a lead was ready.",
          email: "Email",
          whatsapp: "WhatsApp",
          both: "Both",
          none: "Not yet notified",
        },
        responseTrend: {
          title: "Response time trend",
          subtitle: "Weekly average — bot speed for the last 8 weeks.",
        },
      },
      leads: {
        title: "Leads",
        none: "No leads",
        filterLabel: "Filter by status",
        filterAll: "All",
        emptyFilteredTitle: "No leads with this status",
        emptyFilteredHint: "Change the filter, or share your WhatsApp link.",
        emptyTitle: "No leads have arrived yet",
        emptyHint: "The first customer who taps your WhatsApp link and asks to be contacted will appear here.",
        loadFailed: "Couldn't load leads",
        statusUpdated: "Status updated",
        statusUpdateFailed: "Couldn't update status",
        viewTable: "Table",
        viewKanban: "Kanban",
        col: {
          id: "#",
          customer: "Customer",
          mobile: "Mobile",
          lastMessage: "Last message",
          listings: "Listings",
          status: "Status",
          when: "When",
        },
        action: {
          contacted: "Contacted",
          close: "Close",
          closedLabel: "Closed",
          markNotified: "Mark notified",
          reopen: "Reopen",
        },
        page: "Page",
        of: "of",
        prev: "← Previous",
        next: "Next →",
        kanbanEmptyColumn: "Nothing here",
      },
      inventory: {
        eyebrow: "Inventory upload",
        title: "Add your properties to the AI",
        body:
          "Upload a CSV file with your properties. Each row joins your office's catalog and starts surfacing to customers on WhatsApp instantly.",
        dropPrompt: "Drop the file here",
        fileReady: "File ready",
        orPick: "Or click to pick",
        sizeMaxHint: "CSV · max 20 MB",
        mustBeCsv: "File must be CSV",
        maxSize: "Max size is 20 MB",
        downloadOk: "Template downloaded",
        downloadFailed: "Couldn't download the template",
        uploadFailed: "Upload failed",
        uploadCta: "Upload",
        switchFile: "← Pick a different file",
        resultEyebrow: "Upload result",
        col: {
          total: "Total",
          succeeded: "Succeeded",
          skipped: "Skipped",
          failed: "Failed",
        },
        errorsHeader: "errors",
        templateEyebrow: "CSV template",
        templateBody:
          "Download an empty file with all required columns, fill it with your properties, then upload.",
        downloadTemplate: "↓ Download template",
        columnsEyebrow: "Required columns",
        columnsTip: "The template also has many optional columns you can use.",
      },
      settings: {
        profileEyebrow: "Profile",
        profileTitle: "Office details",
        businessName: "Office name",
        notifEmail: "Notification email",
        brokerPhone: "Broker WhatsApp number",
        channel: "Notification channel",
        save: "Save changes",
        loadFailed: "Couldn't load settings",
        saveFailed: "Couldn't save settings",
        saved: "Settings saved",
        magicLinkEyebrow: "Your WhatsApp link",
        magicLinkBody:
          "Share it in your ads. Every customer who taps it lands at your office directly.",
        copyLink: "Copy link",
        copyOk: "Link copied",
        copyFailed: "Couldn't copy",
        openWhatsapp: "Open in WhatsApp ↗",
        accountInfo: "Account info",
        plan: "Plan",
        status: "Status",
        active: "Active",
        suspended: "Suspended",
        brokerId: "Broker ID",
      },
      status: {
        new: "New",
        notified: "Notified",
        contacted: "Contacted",
        closed: "Closed",
      },
      notifications: {
        title: "Notifications",
        aria: "Notifications",
        empty: "All caught up — nothing new.",
        unreadShort: "unread",
        markRead: "Mark read",
        dismiss: "Dismiss",
        markFailed: "Couldn't mark as read",
        dismissFailed: "Couldn't dismiss",
        type: {
          HotLead: "Hot lead",
          DifficultSituation: "Needs you",
          AngryCustomer: "Frustrated",
        },
      },
      chat: {
        title: "Chat session",
        turns: "turns",
        loadFailed: "Couldn't load the chat",
        empty: "No turns recorded for this session yet.",
        debugLabel: "Debug",
      },
      takeover: {
        takeover: "Take over",
        handback: "Hand back to bot",
        success: "You're handling this chat now",
        handbackSuccess: "Bot is handling this again",
        failed: "Couldn't switch handler",
        handlingBadge: "Broker handling",
        handlingHint: "Bot is paused. Type to the customer directly via WhatsApp.",
        botHandlingHint: "The bot is replying. Click Take over to step in.",
      },
      knowledge: {
        title: "Market knowledge",
        subtitle: "Verified answers your bot uses for expert questions (mortgage, tax, شهر عقاري, etc.).",
        loadFailed: "Couldn't load entries",
        topicFilter: "Topic",
        topicAll: "All topics",
        add: "Add entry",
        addTitle: "New knowledge entry",
        editTitle: "Edit entry",
        emptyTitle: "No entries yet",
        emptyHint: "Add your first entry — or fork a global default to customize for your tenant.",
        col: {
          topic: "Topic",
          question: "Question",
          answer: "Answer",
          scope: "Scope",
          verified: "Last verified",
          region: "Region",
        },
        topicValue: {
          Mortgage: "Mortgage",
          Tax: "Tax",
          Documentation: "Documentation",
          PaymentPlan: "Payment plans",
          Compound: "Compounds",
          NorthCoast: "North Coast",
          ForeignOwnership: "Foreign ownership",
          Brokerage: "Brokerage",
          Pricing: "Pricing",
          Process: "Process",
          Other: "Other",
        },
        scopeGlobal: "Global default",
        scopeMine: "My override",
        inactive: "inactive",
        fork: "Fork",
        edit: "Edit",
        delete: "Delete",
        forkSuccess: "Forked — you can now edit your copy.",
        forkFailed: "Couldn't fork the entry",
        deleteSuccess: "Entry deleted",
        deleteFailed: "Couldn't delete the entry",
        confirmDelete: "Delete this knowledge entry?",
        requiredFields: "Question and Answer are required",
        updateSuccess: "Saved — re-indexing in the background.",
        createSuccess: "Created — re-indexing in the background.",
        saveFailed: "Couldn't save",
        regionHelper: "Optional — leave empty for all regions.",
        sourceUrl: "Source URL",
        sourceHelper: "Authoritative source so the broker can verify.",
        confidence: "Confidence (0.0 – 1.0)",
        confidenceHelper: "Below 0.6 the bot tells the customer the info is tentative.",
      },
    },
  },
  ar: {
    common: {
      brand: "Property-Agent",
      tagline: "وكيلك العقاري الذكي على واتساب",
      back: "الرجوع",
      cancel: "إلغاء",
      save: "حفظ",
      submit: "إرسال",
      continue: "التالي",
      loading: "جارٍ التحميل…",
      retry: "حاول تاني",
      copy: "نسخ",
      copied: "تم النسخ",
      learnMore: "اعرف أكتر",
      egypt: "القاهرة",
    },
    nav: {
      home: "الرئيسية",
      pricing: "الأسعار",
      login: "سجّل الدخول",
      signup: "سجّل وسيط",
      signOut: "تسجيل الخروج",
      getStarted: "ابدأ مجاناً",
      dashboard: "اللوحة",
    },
    languageToggle: {
      label: "اللغة",
      switchTo: "English",
      currentEn: "EN",
      currentAr: "AR",
    },
    hero: {
      eyebrow: "وكيلك العقاري الذكي على واتساب",
      titlePart1: "بوت يردّ على عملائك،",
      titleAccent: "ويسلّمك الفرص الجاهزة",
      titlePart2: "في ثواني.",
      body:
        "اربط مخزونك في 30 ثانية. البوت يكلّم العميل بالعربي المصري بمستوى وسيط قاهري متمرّس، ولمّا يكون جاهز يبعتلك تنبيه فوراً.",
      ctaPrimary: "ابدأ مجاناً — 30 ثانية",
      ctaSecondary: "عندك حساب؟ سجّل الدخول",
      pill1Title: "فرص أكثر، وقت أقل",
      pill1Body: "محادثات حقيقية مع عملاء حقيقيين، طول اليوم.",
      pill2Title: "ردود ذكية وطبيعية",
      pill2Body: "بيرد زي ما الوسيط الكبير بيرد — مش روبوت.",
      pill3Title: "على واتساب",
      pill3Body: "في المكان اللي عميلك أصلاً موجود فيه. من غير تطبيق جديد.",
      badge1: "إعداد في 30 ثانية",
      badge2: "ردود بالعربي المصري",
      badge3: "تنبيه إيميل وواتساب",
      mockup: {
        contact: "Property-Agent",
        contactStatus: "متصل الآن",
        today: "اليوم",
        greeting: "أهلاً 👋 إزاي أقدر أساعدك النهارده؟ ممكن أعرف اسم حضرتك؟",
        greetingTime: "11:40 ص",
        msg1Customer: "أنا أحمد — عايز شقة 3 غرف في التجمع، حوالي 4 مليون",
        msg1Time: "11:42 ص",
        msg2Bot: "تشرّفنا يا أحمد. عندي 4 خيارات تناسب ميزانيتك — أبعتلك أول واحدة؟",
        msg2Time: "11:42 ص",
        msg3Customer: "أيوه من فضلك",
        msg3Time: "11:43 ص",
        msg4ListingTitle: "التجمع الخامس",
        msg4ListingArea: "165 م",
        msg4ListingRooms: "3 غرف · 2 حمام",
        msg4ListingPrice: "3,800,000 جنيه",
        msg4ListingTag: "إطلالة جاردن",
        msg5Bot: "بتشمل جاردن 12 م، جراج لـ 2 عربية، وحمام سباحة جنب العمارة.",
        msg5Time: "11:43 ص",
        msg6Customer: "حلوة دي فعلاً 💚",
        msg6Time: "11:44 ص",
        msg7Bot: "تمام، إمتى يناسبك تشوفها؟",
        msg7Time: "11:44 ص",
        msg8Customer: "بكرة الصبح يناسبني",
        msg8Time: "11:44 ص",
        msg9Bot: "اضغط هنا تأكّد الميعاد — وكيلك هيكون مستني 🗓️",
        msg9Time: "11:45 ص",
        ctaButton: "احجز المعاينة",
        msg10Customer: "أيوه، احجزلي من فضلك",
        msg10Time: "11:46 ص",
        msg11Bot: "تمام يا فندم ✅ تم حجز معاينتك. فريقنا هيتواصل معاك قريب لتأكيد الموعد.",
        msg11Time: "11:46 ص",
        composerHint: "اكتب رسالة",
        successTitle: "تم حجز المعاينة!",
        successWhen: "بكرة · 9:30 ص",
        successPhone: "+20 10 XXX XXXX",
        successFooter: "تم إضافة العميل للوحة المتابعة",
      },
    },
    how: {
      eyebrow: "كيف يشتغل",
      title: "ثلاث خطوات تفصلك عن أوّل عميل.",
      step1Title: "سجّل ورفّع المخزون",
      step1Body:
        "املأ نموذج التسجيل، ارفع ملف العقارات (CSV)، واختار قناة التنبيه — إيميل أو واتساب.",
      step1Note: "بنبعتلك مفتاح الوسيط ورابط واتساب جاهز للنشر.",
      step2Title: "انشر رابط واتساب",
      step2Body:
        "رابط فيه كود وسيطك الخاص. حطه في إعلاناتك، الـ Stories، الكروت — كل عميل يفتحه يوصل لك مباشرة.",
      step2Note: "بدون أي إعداد على واتساب من ناحيتك.",
      step3Title: "استلم العملاء الجاهزين",
      step3Body:
        "البوت يفهم العميل، يرشحله من عقاراتك، ولما يكون جاهز يقولك بإيميل أو واتساب فيه رقمه وآخر رسالة منه.",
      step3Note: "لوحة فيها كل العملاء، فلترة حسب الحالة، وأزرار سريعة.",
      quote:
        "«الفرق بين الوسيط الكويس والمتميّز هو السرعة في الرد. البوت بيرد في تانية وبيسلّمك الجاهزين بس.»",
      ctaStart: "ابدأ التسجيل ←",
    },
    footer: {
      city: "Property-Agent · القاهرة",
      version: "v1",
    },
    auth: {
      loginEyebrow: "تسجيل الدخول",
      loginTitlePart: "ارجع",
      loginTitleAccent: "للوحة وسيطك",
      loginBody:
        "ادخل بالإيميل وكلمة المرور اللي اخترتهم وقت التسجيل. هنرجّعك على لوحتك في ثواني.",
      noAccountQuestion: "لسه ما عندكش حساب؟",
      noAccountCta: "سجل وسيط جديد",
      credsTitle: "الإيميل وكلمة المرور",
      credsEyebrow: "بياناتك",
      emailLabel: "الإيميل",
      passwordLabel: "كلمة المرور",
      loginSubmit: "ادخل",
      signinSuccess: "أهلاً بيك من جديد",
      apiKeyEyebrow: "طريقة بديلة",
      apiKeyTitle: "دخول بمفتاح API",
      apiKeyBody:
        "للحسابات القديمة اللي ماعندهاش كلمة مرور بعد. استخدم نفس المفتاح اللي ظهر بعد التسجيل.",
      apiKeySubmit: "ادخل بالمفتاح",
      apiKeySwitch: "دخول بمفتاح API بدلاً عن ذلك ←",
      apiKeyInvalid: "المفتاح غير صحيح أو لم يعد ساري",
      genericError: "حصلت مشكلة، حاول تاني",
      wrongCreds: "الإيميل أو كلمة المرور غير صحيحة",

      signupEyebrow: "إنشاء حساب وسيط",
      signupTitlePart: "ابني",
      signupTitleAccent: "محرّك العملاء",
      signupTitleEnd: "في أقل من دقيقة",
      signupBody:
        "إيميل، كلمة مرور، اسم النشاط، تليفون، قناة. أوّل ما تبعت بنبعتلك مفتاح الوسيط ورابط واتساب جاهز.",
      businessName: "اسم النشاط",
      brokerEmail: "الإيميل",
      brokerPhone: "التليفون (بكود الدولة)",
      passwordPlaceholder: "8 حروف أو أكتر",
      confirmPassword: "تأكيد كلمة المرور",
      channel: "قناة التنبيه",
      channelEmail: "إيميل",
      channelWhatsApp: "واتساب",
      channelBoth: "الاتنين",
      submitSignup: "أنشئ حسابي",
      passwordsMustMatch: "كلمتا المرور غير متطابقتين",
      passwordTooShort: "على الأقل 8 حروف",
      emailTaken: "الإيميل ده عنده حساب بالفعل",
      signupSuccess: "أهلاً بيك",

      successTitle: "تمام — لوحتك جاهزة",
      successBody:
        "دا رابطك الذكي. انشره في أي حتة وكل عميل بيدوس عليه بيتحوّل لبوتك مباشرة.",
      whatsappCta: "افتح على واتساب",
      goToDashboard: "افتح اللوحة",
      step1: "الحساب",
      step2: "النشاط",
      step3: "التنبيه",
      stepOf: "خطوة {current} من {total}",
      next: "التالي",
      previous: "→ السابق",
      stepAccountTitle: "اعمل حسابك",
      stepAccountBody: "إيميل + كلمة مرور — هتستخدمهم كل ما تدخل لوحتك.",
      stepBusinessTitle: "بيانات مكتبك",
      stepBusinessBody: "محتاجين البيانات دي عشان نوصّل العملاء لك باسم النشاط.",
      stepChannelTitle: "نوصّلك إزاي؟",
      stepChannelBody: "بنبعتلك تنبيه ساعة ما العميل يكون جاهز.",
    },
    dashboard: {
      eyebrow: "لوحة الوسيط",
      title: "مكتبك على",
      titleAccent: "واتساب",
      tabStats: "نظرة عامة",
      tabLeads: "العملاء",
      tabInventory: "المخزون",
      tabKnowledge: "المعرفة",
      tabAnalytics: "التحليلات",
      tabSettings: "الإعدادات",
      loadFailed: "ما قدرتش أحمّل البيانات",
      shareLink: "رابط الواتساب",
      copyLinkShort: "نسخ الرابط",
      pageTitle: {
        overview: "نظرة عامة",
        leads: "العملاء",
        inventory: "المخزون",
        knowledge: "المعرفة السوقية",
        analytics: "التحليلات",
        settings: "الإعدادات",
      },
      pageSubtitle: {
        overview: "ملخّص نشاطك، العملاء العاجلين، وقمع التحويل.",
        leads: "كل العملاء اللي وصلوا لك من البوت.",
        inventory: "عقاراتك — اللي البوت يرشّحها للعملاء.",
        knowledge: "الإجابات المعتمدة اللي البوت يبني عليها للأسئلة المتخصصة.",
        analytics: "تحليلات أعمق — القنوات، وقت الرد، توزيع الحالات.",
        settings: "بيانات المكتب وإعدادات التنبيهات.",
      },
      sidebar: {
        collapse: "إغلاق",
        expand: "فتح",
      },
      stats: {
        today: "آخر 24 ساعة",
        last7: "آخر 7 أيام",
        last30: "آخر 30 يوم",
        notifySuccess: "نسبة التنبيه الناجحة",
        todayEmpty: "لسه مفيش عملاء النهارده",
        noNotificationsYet: "ما حصلش تنبيه بعد",
        recentTitle: "آخر عملاء وصلوك",
        recentEnLabel: "Recent activity",
        emptyTitle: "مفيش عملاء بعد",
        emptyHint: "أول ما عميل يطلب يتواصل معاك، هيظهر هنا.",
        unknownCustomer: "عميل",
        hotLeads: "عملاء عاجلين",
        hotLeadsHint: "محتاجين رد دلوقتي",
        totalLeads: "إجمالي العملاء",
        totalLeadsHint: "كل الفترة",
        conversionRate: "نسبة التحويل",
        conversionRateHint: "مغلق / الإجمالي",
        avgResponse: "متوسط الرد",
        avgResponseHint: "من جديد → اتنبه",
        closedMtd: "مغلق (الشهر)",
        closedMtdHint: "الشهر الحالي",
        vsYesterday: "عن إمبارح",
        vsLastWeek: "عن الأسبوع اللي فات",
        vsLastMonth: "عن الشهر اللي فات",
        seconds: "ث",
        minutes: "د",
        hours: "س",
        days: "يوم",
        noData: "—",
      },
      funnel: {
        eyebrow: "مسار العميل",
        title: "قمع التحويل",
        empty: "مفيش عملاء لسه — القمع هيظهر أول ما يبدأ العملاء يوصلوا.",
        stage: {
          received: "استلام",
          notified: "تنبيه",
          contacted: "تواصل",
          closed: "إغلاق",
        },
        dropFromPrev: "−{n}",
      },
      analytics: {
        loading: "جاري حساب الأرقام…",
        empty: "مفيش بيانات لسه — الرسوم هتظهر أول ما يبدأ العملاء يوصلوا.",
        leadsPerDay: {
          title: "العملاء يومياً",
          subtitle: "آخر 30 يوم · عدد العملاء الجدد كل يوم.",
        },
        statusMix: {
          title: "توزيع الحالات",
          subtitle: "إزاي العملاء موزعين دلوقتي.",
        },
        channelMix: {
          title: "قنوات التنبيه",
          subtitle: "البوت نبّهك إزاي لما العميل كان جاهز.",
          email: "إيميل",
          whatsapp: "واتساب",
          both: "الاتنين",
          none: "لسه ما تنبهتش",
        },
        responseTrend: {
          title: "اتجاه وقت الرد",
          subtitle: "متوسط أسبوعي · سرعة البوت آخر 8 أسابيع.",
        },
      },
      leads: {
        title: "العملاء",
        none: "ما فيش عملاء",
        filterLabel: "فلترة حسب الحالة",
        filterAll: "كل الحالات",
        emptyFilteredTitle: "ما فيش عملاء بالحالة دي",
        emptyFilteredHint: "غيّر الفلتر، أو ابدأ بنشر رابط الواتساب الخاص بيك.",
        emptyTitle: "لسه ما وصلكش عملاء",
        emptyHint:
          "أول عميل يفتح رابط الواتساب الخاص بيك ويطلب يتواصل، هيظهر هنا.",
        loadFailed: "ما قدرتش أحمّل العملاء",
        statusUpdated: "تم تحديث الحالة",
        statusUpdateFailed: "ما قدرتش أحدّث الحالة",
        viewTable: "جدول",
        viewKanban: "بورد",
        col: {
          id: "#",
          customer: "العميل",
          mobile: "الموبايل",
          lastMessage: "آخر رسالة",
          listings: "عقارات",
          status: "الحالة",
          when: "وصل",
        },
        action: {
          contacted: "تواصلت",
          close: "أغلق",
          closedLabel: "مغلق",
          markNotified: "تنبيه",
          reopen: "إعادة فتح",
        },
        page: "صفحة",
        of: "من",
        prev: "→ السابق",
        next: "التالي ←",
        kanbanEmptyColumn: "فاضي",
      },
      inventory: {
        eyebrow: "رفع المخزون",
        title: "ارفع عقاراتك على المنصة",
        body:
          "ارفع ملف CSV فيه عقاراتك. كل صف يضاف لمخزون مكتبك ويبدأ يظهر للعملاء على واتساب فوراً.",
        dropPrompt: "اسحب الملف هنا",
        fileReady: "ملف جاهز",
        orPick: "أو اضغط للاختيار",
        sizeMaxHint: "CSV · أقصى 20 ميجا",
        mustBeCsv: "الملف لازم يكون CSV",
        maxSize: "الحد الأقصى 20 ميجا",
        downloadOk: "تم تحميل النموذج",
        downloadFailed: "ما قدرتش أنزّل النموذج",
        uploadFailed: "حصلت مشكلة في الرفع",
        uploadCta: "ارفع المخزون",
        switchFile: "اختر ملف تاني",
        resultEyebrow: "نتيجة الرفع",
        col: {
          total: "إجمالي",
          succeeded: "نجح",
          skipped: "مكرر/متجاهل",
          failed: "فشل",
        },
        errorsHeader: "خطأ",
        templateEyebrow: "نموذج CSV",
        templateBody:
          "نزّل ملف فاضي بكل الأعمدة المطلوبة، املاه بعقاراتك، وارفعه.",
        downloadTemplate: "↓ تحميل النموذج",
        columnsEyebrow: "الأعمدة الأساسية",
        columnsTip: "في النموذج أعمدة اختيارية كتير تقدر تستخدمها.",
      },
      settings: {
        profileEyebrow: "الملف الشخصي",
        profileTitle: "بيانات المكتب",
        businessName: "اسم المكتب",
        notifEmail: "إيميل التنبيهات",
        brokerPhone: "رقم واتساب الوسيط",
        channel: "قناة التنبيه",
        save: "حفظ التعديلات",
        loadFailed: "ما قدرتش أحمّل الإعدادات",
        saveFailed: "ما قدرتش أحفظ الإعدادات",
        saved: "تم حفظ الإعدادات",
        magicLinkEyebrow: "رابط واتساب الخاص بيك",
        magicLinkBody: "انشره في إعلاناتك. كل عميل يفتحه يوصل لمكتبك مباشرة.",
        copyLink: "نسخ الرابط",
        copyOk: "تم نسخ الرابط",
        copyFailed: "ما قدرتش أنسخ",
        openWhatsapp: "افتح في واتساب ↗",
        accountInfo: "معلومات الحساب",
        plan: "الباقة",
        status: "الحالة",
        active: "نشط",
        suspended: "موقوف",
        brokerId: "رقم الوسيط",
      },
      status: {
        new: "جديد",
        notified: "تم التنبيه",
        contacted: "تم التواصل",
        closed: "مغلق",
      },
      notifications: {
        title: "التنبيهات",
        aria: "التنبيهات",
        empty: "كل حاجة تمام — مفيش جديد.",
        unreadShort: "غير مقروء",
        markRead: "علّم مقروء",
        dismiss: "تجاهل",
        markFailed: "ما قدرتش أعلّمها مقروءة",
        dismissFailed: "ما قدرتش أتجاهلها",
        type: {
          HotLead: "عميل ساخن",
          DifficultSituation: "محتاج تدخّل",
          AngryCustomer: "عميل متضايق",
        },
      },
      chat: {
        title: "محادثة العميل",
        turns: "ردود",
        loadFailed: "ما قدرتش أحمّل المحادثة",
        empty: "مفيش ردود مسجّلة لهذه الجلسة.",
        debugLabel: "تفاصيل تقنية",
      },
      takeover: {
        takeover: "أنا هتولّى",
        handback: "رجّع للبوت",
        success: "أنت اللي بتتعامل مع المحادثة دلوقتي",
        handbackSuccess: "البوت رجع يتعامل",
        failed: "ما قدرتش أبدّل المعالج",
        handlingBadge: "الوسيط بيتعامل",
        handlingHint: "البوت متوقف. اكتب للعميل مباشرة من واتساب.",
        botHandlingHint: "البوت بيرد. لو عايز تتدخّل اضغط «أنا هتولّى».",
      },
      knowledge: {
        title: "المعرفة السوقية",
        subtitle: "إجابات معتمدة بيستخدمها البوت للأسئلة المتخصصة (تمويل، ضرايب، شهر عقاري...).",
        loadFailed: "ما قدرتش أحمّل المدخلات",
        topicFilter: "الموضوع",
        topicAll: "كل المواضيع",
        add: "إضافة مدخل",
        addTitle: "مدخل معرفة جديد",
        editTitle: "تعديل المدخل",
        emptyTitle: "مفيش مدخلات لسه",
        emptyHint: "ابدأ بإضافة مدخل جديد — أو انسخ مدخل عام عشان تخصّصه لمكتبك.",
        col: {
          topic: "الموضوع",
          question: "السؤال",
          answer: "الإجابة",
          scope: "النطاق",
          verified: "آخر تحقّق",
          region: "المنطقة",
        },
        topicValue: {
          Mortgage: "تمويل عقاري",
          Tax: "ضرايب",
          Documentation: "التوثيق",
          PaymentPlan: "خطط القسط",
          Compound: "كمبوندات",
          NorthCoast: "الساحل الشمالي",
          ForeignOwnership: "تمليك للأجانب",
          Brokerage: "العمولة",
          Pricing: "التسعير",
          Process: "إجراءات",
          Other: "أخرى",
        },
        scopeGlobal: "افتراضي عام",
        scopeMine: "تخصيص خاص",
        inactive: "غير مفعّل",
        fork: "نسخ وتخصيص",
        edit: "تعديل",
        delete: "حذف",
        forkSuccess: "تم النسخ — تقدر تعدّل النسخة دي.",
        forkFailed: "ما قدرتش أنسخ المدخل",
        deleteSuccess: "تم حذف المدخل",
        deleteFailed: "ما قدرتش أحذف المدخل",
        confirmDelete: "حذف المدخل ده؟",
        requiredFields: "السؤال والإجابة مطلوبين",
        updateSuccess: "تم الحفظ — جاري إعادة الفهرسة في الخلفية.",
        createSuccess: "تم الإنشاء — جاري الفهرسة في الخلفية.",
        saveFailed: "ما قدرتش أحفظ",
        regionHelper: "اختياري — سيبه فاضي لكل المناطق.",
        sourceUrl: "رابط المصدر",
        sourceHelper: "مصدر موثوق عشان الوسيط يقدر يتأكّد منه.",
        confidence: "درجة الثقة (0.0 – 1.0)",
        confidenceHelper: "تحت 0.6 البوت بيقول للعميل إن المعلومة تأكيدية.",
      },
    },
  },
};

/* ────────────────────────────────────────────────────────────────
   Lookup
   ──────────────────────────────────────────────────────────────── */

/** Resolve a dotted path like "auth.loginSubmit" against a locale tree. */
export function lookup(locale: Locale, key: string): string {
  const parts = key.split(".");
  let node: Leaf | Tree | undefined = dict[locale];
  for (const p of parts) {
    if (node && typeof node === "object" && p in node) {
      node = (node as Tree)[p];
    } else {
      node = undefined;
      break;
    }
  }
  if (typeof node === "string") return node;
  // Fallback to English if the AR translation is missing.
  if (locale !== "en") return lookup("en", key);
  return key; // last resort: surface the key so we notice
}
